...
function customEntries(obj) {
  ...
  const customEntriesArr = Object.entries(obj);

  return customEntriesArr;
...
function customObjectFromEntries(entries) {
  ...
  const createdObject = Object.fromEntries(processedEntries);
...
function extendAndFreezeObject(obj1, obj2) {
  ...
  const mergedObjectFreeze = Object.freeze(mergedObject);

  // console.log(Object.isFrozen(mergedObject))

  if (!Object.isFrozen(mergedObject)) {
    return `Помилка: об'єкт не було "заморожено".`
  };

  return mergedObjectFreeze;
}
...
function filterObjectsByKey(arr, key) {
  ...
  const filteredArray = arr.filter(object => object.hasOwnProperty(key));
...
function getObjectValuesSum(obj) {
  ...
  const getValuesSum = getValues.reduce((accumulator, currentValue) => accumulator + currentValue, 0);
